...
const payload = {
  ...
  cf?.addEventListener("submit",async e=>{
    ...
    try{const r=await fetch(`${GOOGLE_SCRIPT_URL}?action=publicConsultar&cpf=${encodeURIComponent(cpf)}&_=${Date.now()}`,{cache:"no-store"});const j=await r.json();const d=unwrap(j);$("consulta-loading").hidden=true;const encontrado=d?.encontrado!==false && (get(d,"numeroInscricao","numero_inscricao","numero")||get(d,"nome","atleta"));if(!encontrado){$("consulta-erro").innerHTML='<strong>CPF NÃO ENCONTRADO</strong><br>Não localizamos uma inscrição para este CPF.';$("consulta-erro").hidden=false;return;}
